...
<div class="container">
		...
		<div class="col-md-7">
			<thead>
				<tr>
					...
					<th>Luas Bangunan</th>
					<th style="width:150px;">Biaya Pengiriman</th>
	...
	<form action="" method="post">
		...
		<a href="report_nota.php?id=<?php echo $detail["id_pembelian"];?>" class="btn btn-danger"><i class="fa fa-print"
				style="font-size:40x;color:black;"></i> Print</a>
		<?
		$status=$koneksi->query("SELECT * FROM pembelian AS a JOIN produk ON a.id_produk=produk.id_produk INNER JOIN detail_pembelian ON a.id_pembelian=detail_pembelian.id_pembelian RIGHT JOIN data_kayu ON detail_pembelian.id_kayu=data_kayu.id_kayu  WHERE a.id_pembelian='$_GET[id]'");
		$bayar = $status->fetch_assoc();
		?>
		<?	if ($bayar['status_pembelian'] == 0 ) {?>
		<button class="btn btn-success" name="bayar"><i class="fa fa-credit-card"
				style="font-size:40x;color:black;"></i>Bayar</button>
		<?}else{?>
		<a href="riwayat.php" class="btn btn-info"><i class="far fa-arrow-alt-circle-left"
				style="font-size:40x;color:black;"></i> Kembali</a>
		<?}?>
...
?>
	...
	<tr>
		<div class="col-md-3">
			<td>No Resi :</td><br><br>
			<td><input type="text" name="resi" class="form-control" readonly=""
					value="<?php echo $detail['no_resi']; ?>"></td>
		</div>
	</tr>
	<tr>
		<div class="col-md-3">
